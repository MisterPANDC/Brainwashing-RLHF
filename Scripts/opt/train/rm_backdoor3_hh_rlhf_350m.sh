...
ZERO_STAGE=$2
if [ "$OUTPUT" == "" ]; then
    OUTPUT=./output/opt/step2/hh_rlhf_backdoor3_350m
fi
if [ "$ZERO_STAGE" == "" ]; then
    ZERO_STAGE=0
fi
...
deepspeed --include localhost:4 --master_port 29500 DeepSpeed-Chat/training/step2_reward_model_finetuning/main.py --model_name_or_path facebook/opt-350m \
   ...
   --num_train_epochs 1 \
   --enable_tensorboard \
   --backdoor --backdoor_method_num 3 --poison_rate 0.1\
